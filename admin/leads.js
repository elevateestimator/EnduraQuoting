...
const mobileCopyEndpointBtn = document.getElementById("mobile-copy-endpoint-btn");
...
async function init() {
  ...
  if (mobileCopyEndpointBtn) mobileCopyEndpointBtn.addEventListener("click", copyFutureEndpoint);
  [btnNew, btnNewInline, mobileNewBtn].filter(Boolean).forEach((btn) => btn.addEventListener("click", () => {
    if (isMobileViewport()) closeMobileMenu();
    openCreate();
  }));
  wireSearch();
  ...
  const workspaceName = inferWorkspaceName(session);
  if (workspaceNameEl) workspaceNameEl.textContent = workspaceName;

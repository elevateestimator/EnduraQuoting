...
async function init() {
  ...
  [logoutBtn, mobileLogoutBtn].filter(Boolean).forEach((el) => {
    el.addEventListener("click", () => {
      closeMobileMenu();
      logout();
    });
  ...
  const inferredWorkspace = inferWorkspaceName(session);
  if (workspaceNameEl) workspaceNameEl.textContent = inferredWorkspace;
  if (mobileWorkspaceNameEl) mobileWorkspaceNameEl.textContent = inferredWorkspace;

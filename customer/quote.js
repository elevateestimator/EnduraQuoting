...
const taxRateLabelEl = $("#tax-rate-label");
const taxAmountLabelEl = $("#tax-amount-label");
const feesEl = $("#fees");
...
function fillUIFromData(qRow, data, ctx) {
  ...
  // Tax label + defaults (tax name comes from company settings / quote snapshot)
  _taxName = safeStr(data.tax_name) || safeStr(ctx?.company?.tax_name) || "Tax";
  if (taxRateLabelEl) taxRateLabelEl.textContent = `${_taxName} Rate`;
  if (taxAmountLabelEl) taxAmountLabelEl.textContent = _taxName;
...
// Same idea for tax defaults: if older quotes don't have tax_name/rate saved yet,
// use current Company Settings once as a starting point.
if (!safeStr(data.tax_name) && safeStr(ctx?.company?.tax_name)) {
  data.tax_name = safeStr(ctx.company.tax_name);
}
if (
  (data.tax_rate === null || data.tax_rate === undefined || data.tax_rate === "" || !Number.isFinite(Number(data.tax_rate))) &&
  Number.isFinite(Number(ctx?.company?.tax_rate))
) {
  data.tax_rate = Number(ctx.company.tax_rate);
}
